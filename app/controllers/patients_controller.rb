class PatientsController < ApplicationController
  ...
  def new
  	@patient = Patient.new
  	@cgdvcode = cgdvcode
  	@title = t('helpers.submit.create', model: Patient.model_name.human)
  end

  def edit
    @patient = Patient.find(params[:id])
    @cgdvcode = @patient.cgdvcode
    @title = t('helpers.submit.update', model: Patient.model_name.human)
  end

  def create
      @patient = Patient.create(resource_params)
  	if @patient.save
  	  flash[:success] = t('flash.success.create', model: Patient.model_name.human)
  	  redirect_to @patient
  	else
  	  @title = "New Patient"
  	  @cgdvcode = cgdvcode
  ...
  def update
    if @patient.update(resource_params)
      flash[:success] = t('flash.success.edit', model: Patient.model_name.human)
      redirect_to @patient
    else
      @title = t('helpers.submit.create', model: Patient.model_name.human)
      @cgdvcode = @patient.cgdvcode
  ...
  def destroy
    ...
    flash[:success] = t('flash.success.destroy', model: Patient.model_name.human)
