...
- [x] Add `config/initializers/new_framework_defaults_6_1.rb` (generated by `app:update`; later stabilization pass enabled a small low-risk subset while keeping high-risk flags deferred).
...
- [x] Record local test evidence in `docs/rails_upgrade_phase2_report.md` (`538 examples, 0 failures, 79 pending` including post-stabilization rerun in `rspec_phase2_rails61_stabilized.json`).
...
- [ ] Enable remaining `new_framework_defaults_6_1.rb` flags one by one with targeted regression checks (high-risk flags still deferred: URL-safe CSRF tokens, legacy connection handling, cookie SameSite, Active Storage variant tracking, etc.).
- [ ] Review/resolve remaining deprecations exposed under Rails 7.0 (notably non-URL-safe CSRF tokens and legacy connection handling) before Phase 3 sign-off.
...
- Phase 2 stabilization pass enabled three low-risk 6.1 defaults and reran the full suite successfully:
  - `active_job.retry_jitter = 0.15`
  - `active_job.skip_after_callbacks_if_terminated = true`
  - `action_view.preload_links_header = true`
- Two higher-impact 6.1 defaults are now applied in `config/application.rb` (not the initializer) for earlier boot behavior under Rails 7:
